...
public class AgendaDAO implements Serializable{
	...
	public List<Agenda> listar() throws SQLException{
		...
		while(rs.next()) {
			...
			agenda.setAgendado(rs.getBoolean("status"));
			agenda.setUsuario(usuarioDAO.consultar(rs.getLong("id")));
			agendas.add(agenda);
		}
		rs.close();
		stmt.close();
	...
	public int alterar(Agenda agenda)throws SQLException{
		if(agenda == null)
			return 0;
		String sql = "UPDATE agenda SET data=?, status=?, idusuario=? WHERE id=?";
		PreparedStatement stmt = (PreparedStatement) con.prepareStatement(sql);
		java.sql.Date data = new java.sql.Date(agenda.getData().getTime());
		stmt.setDate(1, data);
		stmt.setBoolean(2, agenda.isAgendado());
	...
	public Agenda consultar(long id) throws SQLException{
		...
		if(rs.first()) {
			...
			agenda.setData(rs.getDate("data"));
			agenda.setAgendado(rs.getBoolean("status"));
			agenda.setUsuario(usuarioDAO.consultar(rs.getLong("idusuario")));
		}
		rs.close();
		stmt.close();
		return agenda;
